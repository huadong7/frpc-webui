  ...
  <div class="proxy-edit-page">
    ...
    <div class="edit-header">
      <nav class="breadcrumb">
        <router-link v-if="isProfileScoped" :to="'/profiles/' + encodeURIComponent(profileName) + '/proxies'" class="breadcrumb-item">Proxies</router-link>
        <router-link v-else to="/proxies?tab=store" class="breadcrumb-item">Proxies</router-link>
...
// Profile-scoped detection
const profileName = computed(() => (route.params.profile as string) || '')
...
const loadProxy = async () => {
  ...
  try {
    const res = isProfileScoped.value
      ? await getProfileProxy(profileName.value, name)
      : await getStoreProxy(name)
    form.value = storeProxyToForm(res)
    await nextTick()
  } catch (err: any) {
    ElMessage.error('Failed to load proxy: ' + err.message)
    const backPath = isProfileScoped.value
      ? '/profiles/' + encodeURIComponent(profileName.value) + '/proxies'
      : '/proxies?tab=store'
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isProfileScoped.value) {
      if (isEditing.value) {
        await updateProfileProxy(profileName.value, form.value.name, data)
        ElMessage.success('Proxy updated')
      } else {
        await createProfileProxy(profileName.value, data)
    ...
    router.push(isProfileScoped.value
      ? '/profiles/' + encodeURIComponent(profileName.value) + '/proxies'
      : '/proxies?tab=store')
  } catch (err: any) {
